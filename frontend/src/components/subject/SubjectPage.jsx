...
export default function SubjectPage(){
    ...
    const [positionUnitData,setPositionUnitData] = useState(false);
    ...
    return(
        ...
        <div className="flex justify-between p-8 max-sm:p-4">
            <div className={`w-1/4 bg-violet-50 rounded-md overflow-hidden max-sm:w-full max-sm:absolute max-sm:h-screen max-sm:bg-opacity-70 max-sm:left-0 max-sm:pt-20 max-sm:px-4 max-sm:text-sm max-sm:rounded-none top-0`} style={{display:`${(!selectedSubjectDialog && window.innerWidth<=700) ? "none" : "block"}`}}>
            ...
            {user =='teacher' && <div>
                    <button className={`bg-violet-700 text-center p-4 text-white w-full`} onClick={()=>setTopPosition("0%")}>Add subject</button>
                </div>}
                <div className="p-4 flex flex-col gap-2">
                <div className="flex flex-col gap-3">
                    {!subjectsData.length ? <div className="text-gray-500 font-mono text-xl m-auto">No Subjects Available</div> : <>
                    {subjectsData.map((subject,index)=><>
                        <button className={`${subjectId === subject._id ? "bg-violet-300" : ""} w-full p-4 rounded-md text-black border-violet-400 border-x border-y`} key={index} id={subject._id} onClick={(e)=>
            ...
            <div className="w-3/4 px-4 max-sm:w-full max-sm:px-0 text-black">
                ...
                {unitData.map((unit,index)=><>
                    <div className="bg-black p-4 w-full flex justify-between content-center rounded-md bg-opacity-5">
                    <div>
                        <div key={unit._id} className="max-sm:text-base text-black">{unit.title}</div>
                        <small key={index} className="text-gray-500">{unit.description}</small>
                    </div>
                    {
                    user == 'teacher' ? <div className="mt-auto flex gap-2">
                    <button className="rounded-md py-2 px-4 bg-violet-900 max-sm:text-sm font-light text-white" id = {unit._id} onClick={(e)=>{
                        HandleEdit(e);
                    }}>Edit</button>
