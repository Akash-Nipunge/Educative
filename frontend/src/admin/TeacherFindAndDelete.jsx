...
const TeacherFindAndDelete = () => {
      ...
      if (teacher) {
        try {
          const { data } = await axios.get(
            `https://educative-backend.onrender.com/api/v1/admin/teachers?query=${teacher}`
          );
  ...
  const handleDeleteTeacher = async (teacher) => {
    try {
      await axios.delete(`https://educative-backend.onrender.com/api/v1/admin/teachers/${teacher._id}`);
